...
function display_system_logs_for_result(public_id, system_id)
{
	document.location = "/system/" + public_id + "/" + system_id;
}
function oborg_perf_eval_switch(tp)
{
	document.location = "https://openbenchmarking.org/test/" + tp + "&eval=" + document.getElementById("perf_eval_dropdown").value + "#metrics";
}
...
function oborg_add_to_vs(category, existing)
{
	document.location = "/vs/" + category + "/" + (existing ? existing + "," : "") + document.getElementById("add_to_vs_dropdown").value;
}
